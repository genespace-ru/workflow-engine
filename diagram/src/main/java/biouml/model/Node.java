...
import biouml.standard.type.ImageShapeHolder;
...
public class Node extends DiagramElement implements ImageShapeHolder
{
    ...
    public @CheckForNull DimensionEx getShapeSize()
    {
        return size;
    }

    public void setShapeSize(DimensionEx size)
    {
        ...
        if( useCustomImage && image != null )
            image.updateScale();
        firePropertyChange( "shapeSize", oldValue, this.size );
    }
    
    //Old method, need refactoring in callers
    public void setShapeSize(Dimension size)
    {
        setShapeSize( new DimensionEx( this, size.width, size.height ) );
    }
    ...
    {
        if( evt.getPropagationId() == getImage() || evt.getPropagationId() == getShapeSize() || evt.getPropagationId() == getCustomStyle() )
            return true;
