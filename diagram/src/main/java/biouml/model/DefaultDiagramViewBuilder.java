...
public class DefaultDiagramViewBuilder implements DiagramViewBuilder
{
    ...
    public CompositeView createImageView(Node node, Graphics g)
    {
        ...
        else
        {
            ...
            DimensionEx nodeSize = node.getShapeSize();
            Dimension size = new Dimension( nodeSize.getWidth(), nodeSize.getHeight() );//imageDescr.getSize();
    ...
    {
        //TODO: commented NodeViewBuilder
    ...
    public boolean createNodeCoreView(CompositeView container, Node node, DiagramViewOptions options, Graphics g)
    {
        ...
        //TODO: commented CreateEdgeAction

        if( /* CreateEdgeAction.EDGE_END_STUB */"edge-end-stub".equals( node.getKernel().getType() ) )
        {
    ...
    public boolean createNoteView(CompositeView container, Node node, DiagramViewOptions options)
    {
        ...
        DimensionEx nodeSize = node.getShapeSize();
        Dimension shapeSize = new Dimension( nodeSize.getWidth(), nodeSize.getHeight() );
        if( shapeSize != null && note.isBackgroundVisible() )
        {
            shapeSize = (Dimension) shapeSize.clone();
    ...
    public boolean createCompartmentCoreView(CompositeView container, Compartment compartment, DiagramViewOptions options, Graphics g)
    {
        ...
        DimensionEx shapeSize = compartment.getShapeSize();

        RectangularShape shape = null;
        switch( compartment.getShapeType() )
        {
            case Compartment.SHAPE_ROUND_RECTANGLE:
                float arc = Math.min( shapeSize.getWidth(), shapeSize.getHeight() ) * 20 / 100.0f;
                shape = new java.awt.geom.RoundRectangle2D.Float( 0, 0, shapeSize.getWidth(), shapeSize.getHeight(), arc, arc );
                break;

            case Compartment.SHAPE_ELLIPSE:
                shape = new java.awt.geom.Ellipse2D.Float( 0, 0, shapeSize.getWidth(), shapeSize.getHeight() );
                break;

            default:
                shape = new Rectangle( 0, 0, shapeSize.getWidth(), shapeSize.getHeight() );
        }
