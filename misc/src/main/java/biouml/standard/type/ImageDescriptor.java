...
public class ImageDescriptor extends Option implements ImageObserver, PropertyChangeListener
{
    ...
    protected void loadImage()
    {
        ...
        if( image != null ) // image is loaded successfully
        {
            ...
            if( this.getParent() instanceof ImageShapeHolder )
            {
                DimensionEx nodeSize = ((ImageShapeHolder) getParent()).getShapeSize();
                scale.setWidth((int)(nodeSize.getWidth() / imageWidth * 100.0));
                scale.setHeight((int)(nodeSize.getHeight() / imageHeight * 100.0));
                ((ImageShapeHolder) getParent())
                        .setShapeSize( new DimensionEx( nodeSize.getParent(), (int) (imageWidth * scale.getWidth() / 100.0), (int) (imageHeight * scale.getHeight() / 100.0) ) );
            }
        ...
        if( evt.getSource() == getScale() && this.path != null )
        {
            if( this.getParent() instanceof ImageShapeHolder )
            {
                if( evt.getPropertyName().equals("width") )
                {
                    ...
                    ((ImageShapeHolder) getParent()).getShapeSize().setWidth( nodeWidth );
                }
                ...
                if( evt.getPropertyName().equals("height") )
                {
                    ...
                    ((ImageShapeHolder) getParent()).getShapeSize().setHeight( nodeHeight );
                }
            }
